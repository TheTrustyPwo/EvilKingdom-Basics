...
//        new SetSpawnCommand().register();
    ...
    private void terminateServers() {
        ...
        if (transmissionSite.getServerName().equals(lobbyName)) {
            final ArrayList<String> kickMessageList = new ArrayList<String>(this.plugin.getComponentManager().getFileComponent().getConfiguration().getStringList("components.network.shutdowns.kick-message").stream().map(string -> StringUtilities.colorize(string)).collect(Collectors.toList()));
