...
public class MuteChatCommand extends CommandHandler {
    ...
    public void onExecution(final CommandSender sender, final String[] arguments) {
        ...
        this.plugin.getComponentManager().getFileComponent().getConfiguration().getStringList("components.chat.commands.mutechat.messages.success.online-players").forEach(string -> Bukkit.getOnlinePlayers().stream().filter(onlinePlayer -> (onlinePlayer.getUniqueId() != player.getUniqueId()) && !LuckPermsUtilities.getPermissionsViaCache(onlinePlayer.getUniqueId()).contains("basics.chat.global.mutechat.bypass")).forEach(onlinePlayer -> onlinePlayer.sendMessage(StringUtilities.colorize(string.replace("%player%", player.getName()).replace("%status%", status)))));
        Bukkit.getOnlinePlayers().stream().filter(onlinePlayer -> (onlinePlayer.getUniqueId() != player.getUniqueId()) && !LuckPermsUtilities.getPermissionsViaCache(onlinePlayer.getUniqueId()).contains("basics.chat.global.mutechat.bypass")).forEach(onlinePlayer -> onlinePlayer.playSound(onlinePlayer.getLocation(), Sound.valueOf(this.plugin.getComponentManager().getFileComponent().getConfiguration().getString("components.chat.commands.mutechat.sounds.success.online-players.sound")), (float) this.plugin.getComponentManager().getFileComponent().getConfiguration().getDouble("components.chat.commands.mutechat.sounds.success.online-players.volume"), (float) this.plugin.getComponentManager().getFileComponent().getConfiguration().getDouble("components.chat.commands.mutechat.sounds.success.online-players.pitch")));
        this.plugin.getComponentManager().getFileComponent().getConfiguration().getStringList("components.chat.commands.mutechat.messages.success.online-bypassers").forEach(string -> Bukkit.getOnlinePlayers().stream().filter(onlinePlayer -> (onlinePlayer.getUniqueId() != player.getUniqueId()) && LuckPermsUtilities.getPermissionsViaCache(onlinePlayer.getUniqueId()).contains("basics.chat.global.mutechat.bypass")).forEach(onlinePlayer -> onlinePlayer.sendMessage(StringUtilities.colorize(string.replace("%player%", player.getName()).replace("%status%", status)))));
